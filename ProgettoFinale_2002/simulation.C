...
void simulation(TString nameFile, int kNevents, bool scatt , int Nnoise){
    
    delete gRandom;
    MyGen *Gen = new MyGen(13013,60);       
    gRandom = Gen;
    ...
    for( int i = 0 ; i<kNevents ; i++ ){
        cout<<"VERTEX n° "<<i<<endl;
    //----------------------------------------------------- Modificare SOLO QUI------------------------------------------------------------------------------------------------------------------------
        point.mult = Gen->genFixMult();        //Gen->genUnifMult(); -> per distr. uniforme;    Gen->genDistrMult(); -> per distr. assegnata;    Gen->genFixMult(); -> per molt. fissata;
        point.X = Gen->genVerX();
        point.Y = Gen->genVerY();
